...
function isValidEmail(arr8: string[]): never | void {
  if (!Array.isArray(arr8)) throw new Error("Тип данных не массив");
  if (
    !arr8.every((el: string) =>
      /^[a-zA-Z0-9]+@[a-z]{2,8}\.[a-z]{2,5}$/gm.test(el)
    )
  )
    throw new Error("Неверный ввод почты");
}

function checkEmail8(arr8: string[]): string[] {
  try {
    isValidEmail(arr8);
    let newArr8: string[] = [];
